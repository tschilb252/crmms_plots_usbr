...
def create_chart_dd(button_label, site_id, charts, data_dir):
    ...
    charts_dd = get_sub_menus("CHARTS", chart_href, chart_menu_dict, sub_menu_dd="")
...
def create_data_dd(button_label, site_id, data, data_dir, meta, data_format):
    ...
    for data_name, data_none in data.items():
        ...
        data_label = get_datatype_name(data_id, meta)
    ...
    data_dd = get_sub_menus(
        f"{data_format.upper()} DATA", data_href, data_menu_dict, sub_menu_dd=""
    )
...
def get_menu_entry(label, href):
    nl = "\n"
    return f'<li><a tabindex="0" href="{href}">' f"<b><i>{label}</b></i></a></li>{nl}"
...
def get_site_submenu_str(data_dir, site_data, site_id, button_label, meta):
    ...
    for datatype, charts in site_data.items():
        if "charts" in datatype:
            charts_dd = create_chart_dd(button_label, site_id, charts, data_dir)

        if "json" in datatype:
            json_dd = create_data_dd(
                button_label, site_id, charts, data_dir, meta, "json"
            )

        if "csv" in datatype:
            csv_dd = create_data_dd(
                button_label, site_id, charts, data_dir, meta, "csv"
            )

    site_submenu_str = "\n".join([i for i in [csv_dd, json_dd, charts_dd] if i])
...
def create_nav(year_str, data_dir, nav_filename=None):
    ...
    for button_label, dd_items in sorted(
        walk_dict.items(), key=lambda x: int(x[0].split("_")[0])
    ):
        if dd_items:
            ...
            for site_name, site_id in sorted(site_name_dict.items()):
                ...
                site_submenu_list.append(
                    get_site_submenu_str(
                        ".", dd_items[site_id], site_id, button_label, meta
                    )
                )
                site_submenu_str = "\n".join(site_submenu_list)
                site_dd = get_sub_menus(
                    site_label, site_path, sub_menu_dd=site_submenu_str
                )
            ...
            folder_button_label = (
                f"{folder_button_label[0]} {folder_button_label[1]}" f" CRMMS Results"
            )
            folder_button = get_button(folder_button_label, sites_dd_str)
    ...
    nl = "\n"
    nav_html_str = f"{get_header_str(year_str)}{nl}{get_updt_str()}{nl}{buttons_str}{nl}{footer_str}"
    write_nav_dict = {
