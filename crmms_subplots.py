...
def update_layout(fig, site_name, units, initial_rng, date_str, watermark=False):
    ...
    fig.update_layout(
        ...
        title_text=(
            f'<b>{site_name} - {date_str}</b>'.upper()
        ),
        autosize=True,
        images=seal_image,
        yaxis1=dict(
            title=dict(text=f'{units[0]}', font=dict(size=11))
        ),
        yaxis2=dict(
            title=dict(text=f'{units[1]}', font=dict(size=11))
        ),
        yaxis3=dict(
            title=dict(text=f'{units[2]}', font=dict(size=11))
        ),
        xaxis1=dict(
            type='date',
            tickformat= "%b '%y",
