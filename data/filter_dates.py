...
def get_args():
    ...
    parser.add_argument(
        "-d", "--dir", help="directory to filter files in", default=DEFAULT_DATA_DIR
    )
    parser.add_argument(
        "-f",
        "--file",
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(
            f"The end_date provided ({args.end_date}) does not match the format YYYY-MM-DD, please try again..."
        )
        sys.exit(1)
    ...
    for csv_path in csv_dir.glob(glob_pattern):
        ...
        df = pd.read_csv(
            csv_path, parse_dates=["Timestep"], infer_datetime_format=True,
        )
        org_lines = len(df.index)
        df.loc[df["Timestep"] > end_date, "Slot Value"] = np.nan
        df.to_csv(csv_path, index=False, date_format=DEFAULT_DT_FRMT)
        print(f"  Replaced values after {end_date:%Y-%m} with nan\n")
