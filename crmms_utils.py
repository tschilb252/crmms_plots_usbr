...
def get_default_js():
    ...
    return [
        ("leaflet", f"{STATIC_URL}/leaflet/js/leaflet.js"),
        ("jquery", bootstrap_dict["jquery"]),
        ("popper", bootstrap_dict["popper"]),
        ("bootstrap", bootstrap_dict["js"]),
        (
            "awesome_markers",
...
def get_default_css():
    ...
    bootstrap_dict = get_bootstrap()
    return [
        ("leaflet_css", f"{STATIC_URL}/leaflet/css/leaflet.css"),
        ("bootstrap_css", bootstrap_dict["css"]),
        ("awesome_markers_font_css", bootstrap_dict["fa"]),
        (
            "awesome_markers_css",
...
def serial_to_trace(df, val_col="value", trace_col="trace", dt_col="datetime"):
    ...
    for trace in sorted(traces):
        df_temp = df[df[trace_col] == trace].drop_duplicates().copy()
        missing_data_len = len(df_out.index) - len(df_temp.index)
        df_out.insert(
            0, trace, np.array(df_temp[val_col].to_list() + [np.nan] * missing_data_len)
        )
...
def get_tier_traces(t1, t2, site_name, datatype):
    ...
    traces = []
    hover_templates = {"storage": "%{text}", "pool elevation": "%{y:,.0f}'"}
    for tier_name, tier_vals in tiers.items():
        text = [f"{tier_vals[i]/1000:0.0f} kaf" for i, year in enumerate(years)]
        trace = go.Scatter(
            x=dates,
            y=tier_vals,
            text=text,
            ...
            mode="lines+markers",
            marker=dict(symbol="triangle-down", color="rgba(20,20,20,0.4)", size=8),
            line=dict(color="rgba(20,20,20,0.2)", dash="dot", width=2, shape="hv"),
        )
        traces.append(trace)
    return traces
    ...
    try:
        ...
        if huc_filter:
            huc_style = lambda x: {
                ...
                "color": "#1f1f1faa",
                "weight": weight
                if x["properties"][f"HUC{level}"].startswith(huc_filter)
                else 0,
            }
        else:
            huc_style = lambda x: {
    ...
    for idx, coord in coord_series.iteritems():
        if not str(coord).isnumeric():
            ...
            for coord_str in coord_strs:
                coord_digit = "".join(
                    [ch for ch in coord_str if ch.isdigit() or ch == "."]
                )
